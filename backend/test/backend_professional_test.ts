...
import { ejecutarQuery, ejecutarSP, Int, NVarChar, Decimal, SqlDate } from '../src/db/base.repo';
...
async function runDeepBackendTest() {
    ...
    try {
        ...
        const userRows = await ejecutarSP('sp_Usuarios_BuscarPorCarnet', { carnet: { valor: TEST_USER.carnet, tipo: NVarChar } });
        ...
        if (userRows.length === 0) {
            ...
            // Recuperar
            const nuevo = await ejecutarSP('sp_Usuarios_BuscarPorCarnet', { carnet: { valor: TEST_USER.carnet, tipo: NVarChar } });
            idUsuario = nuevo[0].idUsuario;
            console.log(`   ✅ Usuario creado exitosamente. ID Interno: ${idUsuario}`);
        } else {
            idUsuario = userRows[0].idUsuario;
            console.log(`   ✅ Usuario encontrado. ID Interno: ${idUsuario} | Carnet: ${userRows[0].carnet}`);
        }
        ...
        const resTarea = await ejecutarSP<{ idTarea: number }>('sp_Tarea_Crear_Carnet', {
        ...
        if (idTarea) {
            ...
            const checkTarea = await ejecutarQuery('SELECT * FROM p_Tareas WHERE idTarea = @id', { id: { valor: idTarea, tipo: Int } });
            console.log(`      -> Verificación BD: CreadorCarnet='${checkTarea[0].creadorCarnet}' | Estado='${checkTarea[0].estado}'`);
        } else {
        ...
        // Verificación
        const checkCheckin = await ejecutarQuery('SELECT * FROM p_Checkins WHERE idCheckin = @id', { id: { valor: idCheckin, tipo: Int } });
        console.log(`      -> Verificación BD: Prioridad1='${checkCheckin[0].prioridad1}' | Energía=${checkCheckin[0].energia}`);
        ...
        */
        ...
        if (tareaEncontrada) {
            console.log(`      -> Tarea creada '${tareaEncontrada.nombre}' aparece en el Dashboard.`);
        } else {
            console.log('      ⚠️ La tarea creada no aparece (¿Tal vez porque no tiene fechaObjetivo hoy? o estado Pendiente?)');
            console.log('      (Nota: sp_Clarity_MiDia filtra por fechaObjetivo <= hoy)');
        }
        ...
        const proyectos = await planningRepo.obtenerProyectosVisibles(idUsuario, { carnet: TEST_USER.carnet });

        console.log(`   ✅ Proyectos visibles recuperados: ${proyectos.length}`);
        if (proyectos.length > 0) {
            console.log(`      -> Ejemplo: ${proyectos[0].nombre} (Estado: ${proyectos[0].estado})`);
        } else {
            console.log('      -> Ningún proyecto visible (Normal si usuario es nuevo y no creó nada)');
        }
