...
import { ejecutarQuery, ejecutarSP, Int, NVarChar, sql } from '../src/db/base.repo';
...
async function runManualTest() {
    ...
    try {
        ...
        let idUsuario = 0;
        const checkUser = await ejecutarQuery('SELECT idUsuario FROM p_Usuarios WHERE carnet = @c', { c: { valor: TEST_USER.carnet, tipo: NVarChar } });
        ...
        if (checkUser.length === 0) {
            ...
            const newUser = await ejecutarQuery('SELECT idUsuario FROM p_Usuarios WHERE carnet = @c', { c: { valor: TEST_USER.carnet, tipo: NVarChar } });
        ...
        // Verificar en BD
        const dbCheckin = await ejecutarQuery('SELECT * FROM p_Checkins WHERE idCheckin = @id', { id: { valor: idCheckin, tipo: Int } });
        if (dbCheckin[0].usuarioCarnet === TEST_USER.carnet && dbCheckin[0].energia === 90) {
        ...
        if (Array.isArray(proyectos) && proyectos.length > 0) {
            console.log(` -> OK: Se obtuvieron ${proyectos.length} proyectos visibles.`);
