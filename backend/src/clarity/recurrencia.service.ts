...
export class RecurrenciaService {
    ...
    async marcarInstancia(
        ...
        const existente = await recurrenciaRepo.obtenerInstanciaPorFecha(idTarea, fechaProgramada);
        ...
        if (existente) {
            ...
            const recurrencia = await recurrenciaRepo.obtenerRecurrenciaPorTarea(idTarea);
     ...
     */
    verificarDiaAplica(
        recurrencia: { tipoRecurrencia: string; diasSemana?: string; diaMes?: number; fechaInicioVigencia: Date; fechaFinVigencia?: Date },
        fecha: Date
    ): boolean {
        // Verificar vigencia
        if (fecha < recurrencia.fechaInicioVigencia) return false;
        if (recurrencia.fechaFinVigencia && fecha > recurrencia.fechaFinVigencia) return false;
        ...
        if (recurrencia.tipoRecurrencia === 'SEMANAL') {
            ...
            const diasPermitidos = recurrencia.diasSemana?.split(',').map(Number) || [];
