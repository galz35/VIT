...
export class ReportsService {
    ...
    async getReporteProductividad(idLider: number, filter?: any) {
        try {
            ...
            const carnetLider = await this.visibilidadService.obtenerCarnetPorId(idLider);
            if (!carnetLider) return { data: [] };

            const equipo = await this.visibilidadService.obtenerCarnetsVisibles(carnetLider);
            ...
            return await ejecutarQuery(sql, { carnets: { valor: carnetsStr, tipo: NVarChar } });
        } catch (error) {
    ...
    async equipoBloqueos(idUsuario: number, fecha: string) {
        try {
            const carnetLider = await this.visibilidadService.obtenerCarnetPorId(idUsuario);
