...
export async function crearTarea(tarea: Partial<any>): Promise<number> {
    ...
    // Actualizar idTareaPadre si viene (Subtareas)
...
/**
 * Elimina una tarea siguiendo reglas de negocio:
 * - Si fue creada HOY y el solicitante es el creador: Borrado físico.
 * - En cualquier otro caso: Soft Delete (activo = 0).
 */
export async function eliminarTarea(idTarea: number, idSolicitante: number, motivo: string = 'Eliminación manual') {
    const tarea = await ejecutarQuery<{ idCreador: number, fechaCreacion: Date }>(`
        SELECT idCreador, fechaCreacion FROM p_Tareas WHERE idTarea = @id
    `, { id: { valor: idTarea, tipo: Int } });

    if (tarea.length === 0) return; // No existe

    const t = tarea[0];
    const esCreador = t.idCreador === idSolicitante;

    const hoy = new Date();
    const creacion = new Date(t.fechaCreacion);
    const esHoy = hoy.toDateString() === creacion.toDateString();

    if (esCreador && esHoy) {
        // Borrado físico total (cascada manual o FKs)
        // Eliminamos dependencias primero si no hay ON DELETE CASCADE definido
        await ejecutarQuery(`DELETE FROM p_TareaAsignados WHERE idTarea = @id`, { id: { valor: idTarea, tipo: Int } });
        await ejecutarQuery(`DELETE FROM p_Bloqueos WHERE idTarea = @id`, { id: { valor: idTarea, tipo: Int } });
        await ejecutarQuery(`DELETE FROM p_CheckinTareas WHERE idTarea = @id`, { id: { valor: idTarea, tipo: Int } });
        // Finalmente la tarea
        await ejecutarQuery(`DELETE FROM p_Tareas WHERE idTarea = @id`, { id: { valor: idTarea, tipo: Int } });
    } else {
        // Soft Delete con Auditoría
        await ejecutarQuery(`
            UPDATE p_Tareas 
            SET activo = 0,
                deshabilitadoPor = @uid,
                fechaDeshabilitacion = GETDATE(),
                motivoDeshabilitacion = @mot
            WHERE idTarea = @id
        `, {
            id: { valor: idTarea, tipo: Int },
            uid: { valor: idSolicitante, tipo: Int },
            mot: { valor: motivo, tipo: NVarChar }
        });
    }
}

// Alias para compatibilidad con código existente
export async function obtenerMisTareas(idUsuario: number, estado?: string, idProyecto?: number, startDate?: Date, endDate?: Date, query?: string) {
    return getTareasUsuario(idUsuario, estado, idProyecto, startDate, endDate, query);
}

export async function getTareasUsuario(idUsuario: number, estado?: string, idProyecto?: number, startDate?: Date, endDate?: Date, query?: string) {
    let sql = `
        SELECT 
    ...
    if (query) {
        sql += ` AND (t.nombre LIKE @q OR t.descripcion LIKE @q)`;
        params.q = { valor: `%${query}%`, tipo: NVarChar };
    }
    if (startDate && endDate) {
        sql += ` AND t.fechaObjetivo >= @startDate AND t.fechaObjetivo <= @endDate`;
        params.startDate = { valor: startDate, tipo: DateTime };
        params.endDate = { valor: endDate, tipo: DateTime };
    }
...
export async function obtenerTareasMultiplesUsuarios(idsUsuarios: number[]): Promise<any[]> {
    ...
    const sql = `
        SELECT 
            ...
            t.comportamiento, t.idGrupo, t.numeroParte,
            p.nombre as proyectoNombre,
            STRING_AGG(CAST(ta.idUsuario AS VARCHAR), ',') as asignadosIds
        FROM p_Tareas t
        LEFT JOIN p_Proyectos p ON t.idProyecto = p.idProyecto
        ...
        GROUP BY 
            t.idTarea, t.nombre, t.descripcion, t.estado, t.prioridad, 
            t.fechaInicioPlanificada, t.fechaObjetivo, t.porcentaje, t.orden,
            t.comportamiento, t.idGrupo, t.numeroParte, p.nombre
    `;

    const rows = await ejecutarQuery<any>(sql);

    return rows.map(row => {
        const asignados = row.asignadosIds
            ? row.asignadosIds.split(',').map((id: string) => ({ idUsuario: Number(id) }))
            : [];

        // Cleanup auxiliar field
        const { asignadosIds, ...rest } = row;
        return {
            ...rest,
            asignados
        };
...
export async function bloquearTarea(dto: any) {
    // 1. Validar si ya existe bloqueo activo para esta tarea
    // Consideramos "Activo" si el estado no es 'Resuelto'
    const existente = await ejecutarQuery(`
        SELECT idBloqueo FROM p_Bloqueos 
        WHERE idTarea = @t AND estado != 'Resuelto'
    `, { t: { valor: dto.idTarea, tipo: Int } });

    if (existente.length > 0) {
        console.warn(`[bloquearTarea] Bloqueo duplicado evitado para tarea ${dto.idTarea}`);
        return { idBloqueo: existente[0].idBloqueo, yaExistia: true };
    }

    // 2. Insertar nuevo bloqueo
    await ejecutarQuery(`
        INSERT INTO p_Bloqueos (idTarea, idOrigenUsuario, idDestinoUsuario, destinoTexto, motivo, accionMitigacion, creadoEn, estado)
        VALUES (@t, @u, @destU, @destT, @mot, @mit, GETDATE(), 'Activo')
        `, {
        t: { valor: dto.idTarea, tipo: Int },
    ...
    // 3. Actualizar estado tarea a Bloqueada
    if (dto.idTarea) {
...
export async function obtenerNotasUsuario(idUsuario: number) {
    return await ejecutarQuery(`
        SELECT * FROM p_Notas 
        WHERE idUsuario = @u 
...
export async function crearNota(nota: { idUsuario: number, titulo: string, content: string, projectId?: number }) {
    await ejecutarQuery(`
        INSERT INTO p_Notas (idUsuario, titulo, contenido, fechaCreacion, fechaModificacion)
        VALUES (@u, @t, @c, GETDATE(), GETDATE())
    `, {
        u: { valor: nota.idUsuario, tipo: Int },
        t: { valor: nota.titulo, tipo: NVarChar },
        c: { valor: nota.content, tipo: NVarChar }
        // projectId not in standard schema yet, ignored for now or can add later
    });
}
...
export async function actualizarNota(idNota: number, nota: { titulo: string, content: string }) {
    await ejecutarQuery(`
        UPDATE p_Notas 
        SET titulo = @t, contenido = @c, fechaModificacion = GETDATE()
        WHERE idNota = @id
    `, {
...
export async function eliminarNota(idNota: number) {
    await ejecutarQuery(`DELETE FROM p_Notas WHERE idNota = @id`, { id: { valor: idNota, tipo: Int } });
}
