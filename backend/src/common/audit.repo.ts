...
export async function listarAuditLogsPorCarnet(carnetSolicitante: string, page: number = 1, limit: number = 50, searchTerm?: string) {
    console.log(`[AUDIT-REPO] Llamando sp_Auditoria_Equipo_PorCarnet_FAST. Carnet: ${carnetSolicitante}, Page: ${page}, Limit: ${limit}`);

    // Safety check
    if (!carnetSolicitante) {
        console.warn('[AUDIT-REPO] Carnet es null/undefined. Retornando vacío preventivo.');
        return [];
    }
...
export async function contarAuditLogsPorCarnet(carnetSolicitante: string, searchTerm?: string) {
    const res = await ejecutarSP<{ total: number }>('sp_Auditoria_Equipo_PorCarnet_Contar', {
...
export async function listarAuditLogs(limit: number, offset: number, filtros: any = {}) {
    ...
    if (filtros.idsUsuarios && Array.isArray(filtros.idsUsuarios) && filtros.idsUsuarios.length > 0) {
    ...
    if (filtros.query) {
        whereClause += ' AND (a.entidadId LIKE @query OR a.accion LIKE @query OR u.nombre LIKE @query)';
