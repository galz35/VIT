...
export class VisibilidadGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (target.kind === 'idUsuario') {
            ...
            if (!carnetObjetivo) {
                throw new ForbiddenException('No se pudo resolver carnet del objetivo.');
            }
    ...
    private async getCarnetSolicitante(req: any, user: any): Promise<string | null> {
        ...
        if (idUsuario) {
            const carnetDb = await this.visibilidadService.obtenerCarnetPorId(idUsuario);
            if (carnetDb) {
