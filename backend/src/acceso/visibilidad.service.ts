...
export class VisibilidadService {
  ...
  private readonly carnetsCache = new Map<string, { exp: number; value: string[] }>();
    ...
    try {
      ...
      const finalList =
        carnets && carnets.length > 0
          ? Array.from(new Set(carnets.map((c) => this.limpiarCarnet(c)).filter(Boolean)))
          : [cleanCarnet];
  ...
  async obtenerEmpleadosVisibles(carnetSolicitante: string): Promise<any[]> {
    ...
    try {
      ...
      this.logger.error(`Error fetching visible employees | ${error?.message || error}`);
      return [];
    }
  }
  ...
  async obtenerCarnetPorId(idUsuario: number): Promise<string | null> {
    try {
      ...
      this.logger.error(`Error resolving carnet for ID ${idUsuario}: ${error.message}`);
      return null;
    }
  }
  ...
  async verificarAccesoPorId(idSolicitante: number, idObjetivo: number): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`Error verifying access by ID | ${error?.message || error}`);
      return false;
    }
  }
  ...
  async obtenerActoresEfectivos(carnetSolicitante: string): Promise<string[]> {
    ...
    try {
      ...
      this.logger.warn(`Error obteniendo actores efectivos | ${error?.message || error}`);
      return [c];
    }
  }
  ...
  async obtenerMiEquipo(carnetSolicitante: string): Promise<any[]> {
    ...
    try {
      ...
      this.logger.error(`Error fetching team for ${c} | ${error?.message || error}`);
      return [];
    }
  }
