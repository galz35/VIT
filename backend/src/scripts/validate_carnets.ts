...
async function main() {
    ...
    try {
        ...
        if (nulos.length > 0) {
            console.error(`❌ ALERTA: Se encontraron ${nulos.length} usuarios activos sin carnet:`);
            console.table(nulos);
        } else {
        ...
        if (duplicados.length > 0) {
            console.error(`❌ ALERTA: Se encontraron ${duplicados.length} carnets duplicados:`);
        ...
        // 3. Resumen Total
        const total = await ejecutarQuery(`SELECT COUNT(*) as c FROM p_Usuarios WHERE activo = 1`);
