...
async function main() {
    ...
    const check1 = await ejecutarQuery(`SELECT * FROM p_Tareas WHERE idTarea = @id`, { id: { valor: id1, tipo: Int } });
    if (check1.length === 0) {
    ...
    const check2 = await ejecutarQuery(`SELECT * FROM p_Tareas WHERE idTarea = @id`, { id: { valor: id2, tipo: Int } });
    if (check2.length > 0) {
