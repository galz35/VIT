...
async function main() {
    console.log('Verificando columnas faltantes en p_Tareas (Auditoría y Subtareas)...');
    try {
        // 1. Audit Columns for Soft Delete
        const auditCols = ['motivoDeshabilitacion', 'deshabilitadoPor', 'fechaDeshabilitacion'];
        for (const col of auditCols) {
            ...
            if (result.length === 0) {
                ...
                await ejecutarQuery(`ALTER TABLE dbo.p_Tareas ADD ${col} ${type} NULL;`);
        ...
        if (resSub.length === 0) {
            ...
            await ejecutarQuery(`ALTER TABLE dbo.p_Tareas ADD ${subtaskCol} INT NULL;`);
