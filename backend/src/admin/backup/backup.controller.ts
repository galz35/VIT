import { Controller, Get, Res, UseGuards, HttpStatus, Logger, InternalServerErrorException } from '@nestjs/common';
...
export class BackupController {
    ...
    @Get('export')
    @ApiOperation({ summary: 'Genera y descarga un script SQL completo (Schema + Data)' })
    async exportBackup(@Res() res: any) {
        try {
            ...
            stream.on('error', (err) => {
                ...
                if (!res.sent) {
                    res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Error al leer el archivo de backup' });
                }
            });
            ...
            stream.on('end', () => {
                ...
                fs.unlink(filePath, (err) => {
                    if (err) this.logger.error(`No se pudo eliminar el archivo temporal: ${filePath}`);
            ...
            throw new InternalServerErrorException('No se pudo generar el archivo de respaldo. Asegúrese de que mssql-scripter esté instalado en el servidor.');
        }
    }
}
