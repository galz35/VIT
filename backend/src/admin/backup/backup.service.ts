...
export class BackupService {
    ...
    async generateSqlBackup(): Promise<string> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.logger.log(`Iniciando generación de script SQL para la base de datos ${db}...`);
            ...
            const child = spawn('python', [
                ...
                '-P', pass,
                '--schema-and-data',
                '--include-objects', 'Table', 'View', 'StoredProcedure', 'UserDefinedFunction'
            ], { shell: true });
            ...
            child.on('close', (code) => {
                ...
                if (code === 0) {
                    ...
                    this.logger.error(`❌ Error al generar backup. Código: ${code}. Salida: ${errorOutput}`);
                    reject(new Error(`mssql-scripter falló con código ${code}: ${errorOutput}`));
                }
            });

            child.on('error', (err) => {
                this.logger.error(`❌ No se pudo iniciar el proceso de backup: ${err.message}`);
                reject(err);
            });
        });
    }
