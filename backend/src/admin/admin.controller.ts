...
export class AdminController {
    ...
    private getUserId(req: any): number {
        ...
        const num = Number(id);
        if (!num || Number.isNaN(num)) throw new ForbiddenException('Token inválido (sin userId).');
        return num;
    }
    ...
    async asignarUsuarioNodo(@Body() dto: UsuarioOrganizacionAsignarDto, @Req() req: any) {
        const actorId = this.getUserId(req);
