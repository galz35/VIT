...
export async function obtenerUsuariosAccessInfo() {
    ...
    return await ejecutarQuery<UsuarioDb & { subordinateCount: number, menuPersonalizado: string | null }>(`
        SELECT 
            u.*,
...
export async function asignarMenuPersonalizado(idUsuario: number, menuJson: string | null) {
    // Upsert config
...
export async function listarUsuarios(pagina: number = 1, limite: number = 50) {
    ...
    const totalRes = await ejecutarQuery<{ total: number }>(`SELECT COUNT(*) as total FROM p_Usuarios`);
...
export async function cambiarRolUsuario(idUsuario: number, rolGlobal: string, idRol?: number) {
    await ejecutarQuery(`
        UPDATE p_Usuarios 
...
export async function obtenerEstadisticasAdmin() {
    const stats = await ejecutarQuery<{ total: number, activos: number, inactivos: number }>(`
        SELECT 
            COUNT(*) as total,
...
export async function listarRoles() {
    return await ejecutarQuery<RolDb>(`SELECT * FROM p_Roles ORDER BY nombre ASC`);
}
...
export async function listarLogs(pagina: number = 1, limite: number = 50) {
    ...
    const totalRes = await ejecutarQuery<{ total: number }>('SELECT COUNT(*) as total FROM p_Logs');
...
export async function obtenerOrganigrama() {
    return await ejecutarQuery<OrganizacionNodoDb>(`SELECT * FROM p_OrganizacionNodos WHERE activo = 1`);
}

export async function asignarUsuarioNodo(idUsuario: number, idNodo: number, rol: string) {
    await ejecutarQuery(`
...
export async function listarAuditLogs(filtro: { page?: number, limit?: number, idUsuario?: number, accion?: string, query?: string }) {
    ...
    if (filtro.query) {
        whereClause += ' AND (datos LIKE @query OR accion LIKE @query OR entidad LIKE @query)';
