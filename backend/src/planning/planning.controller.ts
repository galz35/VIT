...
export class PlanningController {
    ...
    async requestChange(
        ...
        const idTarea = Number(body?.idTarea);
        if (!idTarea || Number.isNaN(idTarea)) throw new BadRequestException('idTarea inválido.');

        const campo = String(body?.campo || '').trim();
        const motivo = String(body?.motivo || '').trim();

        if (!campo) throw new BadRequestException('campo requerido.');
        if (!motivo || motivo.length < 5) throw new BadRequestException('motivo muy corto.');
    ...
    async getPendingRequests(@Request() req: any) {
        return await this.planningService.getSolicitudesPendientes(this.getUserId(req));
    }
    ...
    async resolveRequest(
        @Request() req: any,
        @Body() body: { idSolicitud: number; accion: 'Aprobar' | 'Rechazar'; comentario?: string },
    ) {
        const userId = this.getUserId(req);

        const idSolicitud = Number(body?.idSolicitud);
        if (!idSolicitud || Number.isNaN(idSolicitud)) throw new BadRequestException('idSolicitud inválido.');
        ...
        return await this.planningService.resolverSolicitud(userId, idSolicitud, accion, body?.comentario);
    }
    ...
    async updateOperative(
        ...
        const idTarea = Number(body?.idTarea);
        if (!idTarea || Number.isNaN(idTarea)) throw new BadRequestException('idTarea inválido.');
        return await this.planningService.updateTareaOperativa(userId, idTarea, body?.updates);
    }
    ...
    async getPlans(
        ...
        if (!objetivo || Number.isNaN(objetivo)) throw new BadRequestException('idUsuario inválido.');
    ...
    @Get('stats')
    async getStats(@Request() req: any, @Query('mes') mes: any, @Query('anio') anio: any) {
        const userId = this.getUserId(req);
        const m = Number(mes);
        const a = Number(anio);
    ...
    async getGlobalCompliance(@Query('mes') mes: any, @Query('anio') anio: any) {
        return await this.analyticsService.getGlobalCompliance(Number(mes), Number(anio));
    }

    @Get('stats/performance')
    async getAreaPerformance(@Query('mes') mes: any, @Query('anio') anio: any) {
        return await this.analyticsService.getAreaPerformance(Number(mes), Number(anio));
    }
    ...
    async reassignTasks(@Body() body: any, @Request() req: any) {
        ...
        const taskIds = Array.isArray(body?.taskIds) ? body.taskIds.map((x: any) => Number(x)) : [];

        if (!fromUserId || Number.isNaN(fromUserId)) throw new BadRequestException('fromUserId inválido.');
        if (!toUserId || Number.isNaN(toUserId)) throw new BadRequestException('toUserId inválido.');
        if (!taskIds.length || taskIds.some((x: number) => !x || Number.isNaN(x))) {
            throw new BadRequestException('taskIds inválido.');
        }

        return await this.planningService.reassignTasks(userId, fromUserId, toUserId, taskIds);
    }

    @Get('tasks/:id/history')
    async getTaskHistory(@Param('id', ParseIntPipe) id: number, @Request() req: any) {
    ...
    async obtenerHistorialMensual(@Param('id', ParseIntPipe) id: number, @Request() req: any) {
        return await this.planningService.obtenerHistorialMensual(id, this.getUserId(req));
    }
    ...
    async agregarFase(
        ...
        if (!idTareaNueva || Number.isNaN(idTareaNueva)) throw new BadRequestException('idTareaNueva inválido.');
        return await this.planningService.agregarFase(idGrupo, idTareaNueva, this.getUserId(req));
    }

    @Get('grupos/:idGrupo')
    @ApiOperation({ summary: 'Obtener todas las fases de un grupo' })
    async obtenerGrupo(@Param('idGrupo', ParseIntPipe) idGrupo: number, @Request() req: any) {
        // service valida visibilidad
        return await this.planningService.obtenerGrupo(idGrupo, this.getUserId(req));
    }

    @Get('dashboard/alerts')
    @ApiOperation({ summary: 'Obtener alertas de dashboard (atrasos y entregas hoy)' })
