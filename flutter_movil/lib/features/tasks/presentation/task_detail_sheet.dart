...
class _TaskDetailSheetState extends State<TaskDetailSheet> {
  ...
  void _initForm() {
    _tituloCtrl = TextEditingController(text: _taskData['titulo']?.toString() ?? '');
    _descripcionCtrl = TextEditingController(text: _taskData['descripcion']?.toString() ?? '');
    _evidenciaCtrl = TextEditingController(text: _taskData['linkEvidencia']?.toString() ?? '');
    _comentarioCtrl = TextEditingController(); // Nuevo comentario siempre vacío al inicio
    
    _estado = _taskData['estado']?.toString() ?? 'Pendiente';
    _prioridad = _taskData['prioridad']?.toString() ?? 'Media';
    _progreso = (_taskData['progreso'] as num?)?.toInt() ?? 0;
    
    if (_taskData['fechaObjetivo'] != null) {
      _fechaObjetivo = DateTime.tryParse(_taskData['fechaObjetivo'].toString());
    }
    if (_taskData['fechaInicioPlanificada'] != null) {
      _fechaInicio = DateTime.tryParse(_taskData['fechaInicioPlanificada'].toString());
    }
  }
    ...
    try {
      ...
      if (mounted && response.data != null) {
        ...
        setState(() {
          ...
          // Better only refresh fields that might be missing in list view
          if (!_hasChanges) {
             _descripcionCtrl.text = data['descripcion'] ?? '';
             _evidenciaCtrl.text = data['linkEvidencia'] ?? '';
              // Fechas
             if (data['fechaObjetivo'] != null) {
                _fechaObjetivo = DateTime.tryParse(data['fechaObjetivo'].toString());
             }
             if (data['fechaInicioPlanificada'] != null) {
                _fechaInicio = DateTime.tryParse(data['fechaInicioPlanificada'].toString());
             }
          }
        });
      }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_comentarioCtrl.text.trim().isNotEmpty) {
        ...
        payload['agregarComentario'] = true; // Flag for backend if needed, or api handles it
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Guardado correctamente'), backgroundColor: Color(0xFF10B981)),
        );
      }
    } catch (e) {
      if (mounted) {
        ...
        children: [
              ...
              children: [
                 ...
                 Expanded(
                   child: TextField(
                     ...
                     maxLines: null,
                     decoration: const InputDecoration.collapsed(hintText: 'Título de la tarea'),
                   ),
                 ),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                if (_isLoadingFull) 
                   const Padding(padding: EdgeInsets.only(bottom: 20), child: LinearProgressIndicator(minHeight: 2)),
                  ...
                  child: Column(
                    children: [
                      Row(
                        children: [
                          Expanded(
                            child: _buildDateInput('Inicio', _fechaInicio, () => _pickDate(true)),
                          ),
                          const SizedBox(width: 16),
                          Expanded(
                            child: _buildDateInput('Objetivo', _fechaObjetivo, () => _pickDate(false)),
                          ),
                        ],
                      ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                       ...
                       // Descripción
                       const Align(alignment: Alignment.centerLeft, child: Text('Descripción', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF64748B)))),
                       const SizedBox(height: 8),
                       TextField(
                         controller: _descripcionCtrl,
                         onChanged: (_) => _hasChanges = true,
                         maxLines: 4,
                         style: const TextStyle(fontSize: 14, fontFamily: 'Inter', color: Color(0xFF334155)),
                         decoration: _cleanInputDeco('Añadir detalles...'),
                       ),

                       const SizedBox(height: 16),
                       // Evidencia
                        const Align(alignment: Alignment.centerLeft, child: Text('Link Evidencia', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF64748B)))),
                       const SizedBox(height: 8),
                       TextField(
                         controller: _evidenciaCtrl,
                         onChanged: (_) => _hasChanges = true,
                         style: const TextStyle(fontSize: 14, fontFamily: 'Inter', color: Color(0xFF2563EB)),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      if (_taskData['comentarios'] != null && (_taskData['comentarios'] as List).isNotEmpty)
                        ...(_taskData['comentarios'] as List).map((c) => _buildCommentItem(c)),
                        
                      TextField(
                        controller: _comentarioCtrl,
                        onChanged: (_) => _hasChanges = true,
                        maxLines: 2,
                        decoration: _cleanInputDeco('Escribe un nuevo comentario o actualización...').copyWith(
                           suffixIcon: const Icon(Icons.send_rounded, size: 18, color: Colors.grey),
                        ),
                      ),
                    ],
                  ),
             ...
             child: SizedBox(
               ...
               child: ElevatedButton(
                 ...
                 style: ElevatedButton.styleFrom(
                   backgroundColor: const Color(0xFF0F172A), // Slate 900 (Black-ish)
                 ...
                 child: _saving 
                    ...
                    : const Text('Guardar y Finalizar', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Inter')),
               ),
             ),
          ),
        ],
  ...
  Widget _buildDateInput(String label, DateTime? value, VoidCallback onTap) {
    final text = value == null ? 'Seleccionar' : DateFormat('d MMM yyyy', 'es').format(value);
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(12),
      child: Column(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(child: Text(text, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF334155)))),
              ],
            ),
          ),
        ],
  ...
  Widget _buildPrioritySelector() {
    return Row(
      ...
      children: [
        const Text('Prioridad', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF475569))),
        Row(
  ...
  IconData _getStatusIcon(String s) {
     switch(s) {
       case 'Hecha': return Icons.check_circle_outline;
       case 'EnCurso': return Icons.timelapse;
       case 'Bloqueada': return Icons.block;
    ...
    return Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text('Progreso', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF64748B))),
            Text('$_progreso%', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF3B82F6))),
          ],
        ),
        const SizedBox(height: 8),
        SliderTheme(
  ...
  Widget _buildCommentItem(dynamic c) {
     ...
     final msg = c['mensaje'] ?? '';
     final date = c['fecha'] != null ? DateFormat('d MMM HH:mm', 'es').format(DateTime.parse(c['fecha'].toString())) : '';
        ...
        child: Column(
           ...
           children: [
              Row(
                 mainAxisAlignment: MainAxisAlignment.spaceBetween,
                 children: [
                    Text(author, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Color(0xFF475569))),
