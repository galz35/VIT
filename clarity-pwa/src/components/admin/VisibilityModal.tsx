...
export const VisibilityModal: React.FC<Props> = ({ user, onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState<'areas' | 'people'>('areas');
    ...
    useEffect(() => {
        if (user?.carnet) fetchPermissions();
    }, [user]);
    ...
    const handleAdd = async (item: any) => {
        ...
        try {
            if (activeTab === 'areas') {
                ...
                console.log('Enviando Permiso Empleado:', { carnetRecibe: user.carnet, carnetObjetivo: item.carnet, tipoAcceso: actionType });
                await clarityService.crearPermisoEmpleado({
                    carnetRecibe: user.carnet,
                    carnetObjetivo: item.carnet,
                    tipoAcceso: actionType // Send ALLOW or DENY
                });
            }
            setSearchQuery('');
            setSearchResults([]);
                ...
                <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden relative">
                    ...
                    <div className="p-6 bg-white border-b border-slate-100 z-10 shrink-0">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            {/* Action Type Toggle (Only for People) */}
                            {activeTab === 'people' && (
                                    ...
                                    <>
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <span className="text-[10px] bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full font-bold">
                                                {(activeTab === 'areas' ? areas : people).filter((p: any) => p.activo !== false).length}
                                            </span>
                                        </div>
