...
export const MenuBuilder: React.FC<MenuBuilderProps> = ({ initialJson, onChange }) => {
    ...
    const generateJson = (paths: Set<string>) => {
        const result = APP_MENU.map(group => {
            // Filter items that are selected
            const activeItems = group.items.filter(item => paths.has(item.path));
            ...
            return {
                ...
                items: activeItems.map(item => ({
                    ...
                    label: item.label,
                    icon: item.icon
                    // Note: We are stripping permissions here for simplicity as the dynamic menu is visibility-based
                    // The backend guard will still check permissions if they try to access the URL directly
                }))
            };
