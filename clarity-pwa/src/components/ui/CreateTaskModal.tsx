...
export const CreateTaskModal: React.FC<Props> = ({ isOpen, onClose, currentProject, projectId, onTaskCreated, onSuccess, defaultTeam }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"
            ...
            {/* ANCHO AUMENTADO A 2XL PARA EVITAR SOLAPE */}
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-200 overflow-hidden">
                            ...
                            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2.5 focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-400 transition-all shadow-sm">
                                ...
                                <div className="flex-1 flex flex-wrap gap-2 min-h-[26px]">
                                    ...
                                    <input
                                        ...
                                        placeholder={selectedAsignados.length === 0 ? "Escribe nombre o carnet..." : ""}
                                        className="bg-transparent outline-none text-xs font-semibold text-slate-700 min-w-[120px] flex-1 placeholder:font-normal"
                                    />
                                </div>
                            </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-3 overflow-hidden">
                                                    <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-xs font-black text-slate-400 uppercase shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-all">
                                                        {(emp.nombreCompleto || (emp as any).nombre || '?').charAt(0)}
                                                    </div>
                                                    <div className="truncate">
                                                        ...
                                                        <p className="text-[10px] text-slate-400 uppercase font-semibold">{emp.cargo || 'Sin Cargo'}</p>
                                                    </div>
                                                </div>
