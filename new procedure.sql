/* =========================================================
   CLARITY / MOMENTUS - STORE PROCEDURES (SQL Server)
   - 100% SP (sin SQL inline en el backend)
   - Compatible con el acceso.repo.ts que te dejé
   - Recomendación: ejecuta en tu DB (dbo)
   ========================================================= */
...
BEGIN
    ...
    /* 
      ...
      - él mismo
      - su subárbol jerárquico por jefeCarnet (p_Usuarios)
      - permisos por empleado (p_permiso_empleado ALLOW)
      - permisos por área (p_permiso_area) => subárbol de organización
      - delegaciones activas: quien me delegó visibilidad acknowledging
        (actores efectivos = yo + delegantes activos)
      
      También soporta DENY en p_permiso_empleado.tipo_acceso='DENY' (si existe)
    */
    ...
    -- Siempre: actores efectivos visibles
    INSERT INTO @Visibles (carnet)
    SELECT carnet FROM @Actores;

    /* A) Jerarquía por jefeCarnet (p_Usuarios): bajar niveles */
    ;WITH Raices AS (
        SELECT u.carnet
    ...
    WHERE pe.activo = 1
      ...
      AND NOT EXISTS (
          SELECT 1 FROM @Visibles v WHERE v.carnet = LTRIM(RTRIM(pe.carnet_objetivo))
      );

    /* C) Permisos por área: subárbol de organización */
    ;WITH PermAreas AS (
    ...
    /* D) Aplicar DENY por empleado (si existe) */
    DELETE v
    FROM @Visibles v
    WHERE EXISTS (
        SELECT 1
    ...
    AND v.carnet <> @carnet; -- el solicitante siempre se ve
...
BEGIN
    ...
    SELECT TOP 1 *
    FROM dbo.p_Usuarios
    WHERE LOWER(LTRIM(RTRIM(correo))) = LOWER(LTRIM(RTRIM(@correo)));
END
GO
...
BEGIN
    ...
    (
        carnet_delegante, carnet_delegado, motivo,
        activo, creado_en, fecha_inicio, fecha_fin
    )
    VALUES
    (
        LTRIM(RTRIM(@delegante)),
        LTRIM(RTRIM(@delegado)),
        @motivo,
        1, GETDATE(), NULL, NULL
    );
...
BEGIN
    ...
    (
        carnet_otorga, carnet_recibe, idorg_raiz, alcance, motivo,
        activo, creado_en, fecha_fin
    )
    VALUES
    (
        CASE WHEN @otorga IS NULL THEN NULL ELSE LTRIM(RTRIM(@otorga)) END,
        LTRIM(RTRIM(@recibe)),
        @idorg,
        @alcance,
        @motivo,
...
BEGIN
    ...
    (
        carnet_otorga, carnet_recibe, carnet_objetivo,
        tipo_acceso, motivo,
        activo, creado_en
    )
    VALUES
...
BEGIN
    ...
    ;WITH NodosSub AS (
        ...
        JOIN NodosSub ns ON n.idPadre = ns.idorg
    )
    SELECT TOP (@limite)
        u.idUsuario,
        u.nombre,
